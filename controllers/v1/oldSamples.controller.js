...
exports.deleteSample = async (req, res) => {
  ...
  try {
    ...
    // 1. Try to find the sample in samplesCollection
    sample = await samplesCollection.findOne({ _id: objectId });
    if (sample) {
      collectionSource = 'samplesCollection';
    } else {
      // 2. If not found in samplesCollection, try in takenSamplesCollection
      sample = await takenSamplesCollection.findOne({ _id: objectId });
      if (sample) {
        collectionSource = 'takenSamplesCollection';
      }
    }
    ...
    // --- Start Position Reduction Logic (if requested and sample is from samplesCollection) ---
    if (collectionSource === 'samplesCollection' && reduceOtherPositions === "yes") {
      ...
      if (isNaN(numericShelf) || isNaN(numericDivision) || isNaN(numericPosition)) {
        ...
        try {
          // Normalization of DB fields (consider moving this to a migration script or a less frequent task)
    ...
    let deleteResult;
    if (collectionSource === 'samplesCollection') {
      deleteResult = await samplesCollection.deleteOne({ _id: objectId });
    } else { // collectionSource === 'takenSamplesCollection'
      deleteResult = await takenSamplesCollection.deleteOne({ _id: objectId });
    }
    ...
    console.error('Error in deleteSample controller:', error);
